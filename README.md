/* =============================================================================================
 * 🚀 SMART SATELLITE SYSTEM - v15.4 (CORRECT BOOT FLOW)
...
// --- QUẢN LÝ BIẾN TOÀN CỤC THỜI TIẾT ---
...
int X_chuChay = 32;                       
int X_chuConnecting = 32; 
unsigned long lastScrollTime = 0;
unsigned long lastScrollTimeConn = 0;
unsigned long waitStartTime = 0;
bool dangChoNghi20s = false;               
String chuoiChayTongHop = "";              
String chuoiChayOwmDungSan = "";           
...
const uint8_t icon_Trang_Khuyet_Nho[] PROGMEM = { 0x3c, 0x70, 0xe0, 0xc0, 0xc0, 0xe0, 0x70, 0x3c };
const uint8_t icon_Set_Cai_Tien[] PROGMEM = { 0x0c, 0x18, 0x30, 0x60, 0xff, 0x38, 0x30, 0x60, 0xc0, 0x80, 0x00, 0x00 };
const uint8_t icon_May_Pixel_Theo_Anh[] PROGMEM = {
  0x07, 0x00, 0x18, 0xe0, 0x23, 0x10, 0x44, 0x08, 0x88, 0x04, 0x90, 0x06, 0x80, 0x01, 0x40, 0x01, 0x22, 0x02, 0x1d, 0x1c, 0x00, 0xe0
};
...
void layThoiTietVeTinhOnline() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin(urlv15); http.setTimeout(4000);
    ...
    if (httpResponseCode == 200) {
      ...
      if (weatherCode == 0) { trangThaiThoiTiet = "NANG"; textTrangThai = "TROI QUANG"; } 
      else if (weatherCode >= 1 && weatherCode <= 3) { trangThaiThoiTiet = "MAY"; textTrangThai = (weatherCode == 1) ? "IT MAY" : "NHIEU MAY"; } 
      else if ((weatherCode >= 51 && weatherCode <= 67) || (weatherCode >= 80 && weatherCode <= 82)) { trangThaiThoiTiet = "MUA"; textTrangThai = "CO MUA GIONG"; } 
      else if (weatherCode >= 95 && weatherCode <= 99) { trangThaiThoiTiet = "DONG_SET"; textTrangThai = "GIONG SET CHOP"; } 

      struct tm timeinfo;
      int gioHienTai = 12;
      if (getLocalTime(&timeinfo)) {
        gioCapNhatAPI = timeinfo.tm_hour; phutCapNhatAPI = timeinfo.tm_min;
        gioHienTaiHeThong = timeinfo.tm_hour; gioHienTai = timeinfo.tm_hour;
      }

      int gioDuBao = (gioHienTai + 1) % 24; int hourlyWeatherCode = weatherCode;  
      if (doc.containsKey("hourly") && doc["hourly"].containsKey("weather_code")) {
        JsonArray hourlyCodes = doc["hourly"]["weather_code"];
        if (gioDuBao < hourlyCodes.size()) { hourlyWeatherCode = hourlyCodes[gioDuBao]; }
      }
      ...
      snprintf(formatCheck, sizeof(formatCheck), " - %s - T:%doC H:%d%% - DU BAO %02dH: %s (METEO SYNC: %02d:%02d)", 
        textTrangThai.c_str(), webTemp, webHum, gioDuBao, duBao1HToi.c_str(), gioCapNhatAPI, phutCapNhatAPI);
...
void veChuyenTrangIconFooter(int x_icon_base, int y_icon, bool checkDem, unsigned long currentMillis) {
  int x = x_icon_base + iconShiftX; int y = y_icon + iconShiftY;
  ...
  if (trangThaiThoiTiet == "NANG") {
    if (checkDem) {
      int x_trang = x - 4; int y_trang = y - 4; display.drawBitmap(x_trang, y_trang, icon_Trang_Khuyet_Nho, 8, 8, SSD1306_WHITE); 
      if ((currentMillis / 500) % 2 == 0) { display.drawPixel(x_trang - 3, y_trang - 2, SSD1306_WHITE); display.drawPixel(x_trang + 11, y_trang + 4, SSD1306_WHITE); }
    } else {
      display.fillCircle(x, y, 3, SSD1306_WHITE); int d = 5 + ((currentMillis / 250) % 2) * 2;
      display.drawPixel(x, y - d, SSD1306_WHITE); display.drawPixel(x, y + d, SSD1306_WHITE); display.drawPixel(x - d, y, SSD1306_WHITE); display.drawPixel(x + d, y, SSD1306_WHITE);
    }
  }
  else if (trangThaiThoiTiet == "MAY") {
    if (checkDem) {
      if (currentMillis - lastStarRandomTime >= 1200) {
        lastStarRandomTime = currentMillis; starRandX1 = random(2, 13); starRandY1 = random(96, 101); starRandX2 = random(18, 30); starRandY2 = random(97, 102);
      }
      if ((currentMillis / 400) % 2 == 0) { display.drawPixel(starRandX1, starRandY1, SSD1306_WHITE); display.drawPixel(starRandX2, starRandY2, SSD1306_WHITE); }
    }
    display.drawBitmap(X_may1, y - 6, icon_May_Pixel_Theo_Anh, 16, 11, SSD1306_WHITE); display.drawBitmap(X_may2, y + 2, icon_May_Pixel_Theo_Anh, 16, 11, SSD1306_WHITE);
  }
  else if (trangThaiThoiTiet == "MUA") {
    int x_may = x - 8; display.drawBitmap(x_may, y - 6, icon_May_Pixel_Theo_Anh, 16, 11, SSD1306_WHITE);
    int quat_roi = (currentMillis / 70) % 6; 
    display.drawPixel(x_may + 3, y + 6 + quat_roi, SSD1306_WHITE); display.drawPixel(x_may + 8, y + 5 + ((quat_roi + 2) % 6), SSD1306_WHITE); display.drawPixel(x_may + 13, y + 7 + ((quat_roi + 4) % 6), SSD1306_WHITE);
  }
  else if (trangThaiThoiTiet == "DONG_SET") {
    int x_may = x - 8; display.drawBitmap(x_may, y - 5, icon_May_Pixel_Theo_Anh, 16, 11, SSD1306_WHITE);
...
void setup() {
  ...
  // 🔥 1. CHÀO TRƯỚC VỚI LOGO TRỐNG TRONG 2 GIÂY ĐẦU
  ...
  display.setCursor(1, 55);  display.print(F("v15.4"));    
  display.drawFastHLine(0, 71, 32, SSD1306_WHITE);
  display.setCursor(1, 78);  display.print(F("READY"));     
  display.display(); 
  delay(2000); // Đứng im chào 2 giây không có chỉ số cảm biến

  // 🔥 2. CHÀO XONG 2 GIÂY MỚI BẮT ĐẦU QUÉT CẢM BIẾN SHT31 VÀ HIỆN THỊ THÔNG SỐ T H%
  if (sht31.begin(0x44)) {
    isSensorOnline = true; rawTemp = sht31.readTemperature(); rawHum = sht31.readHumidity();
    if (!isnan(rawTemp) && !isnan(rawHum)) { filteredTemp = rawTemp; filteredHum = rawHum; }
  } else { isSensorOnline = false; }

  if (isSensorOnline && !isnan(filteredTemp) && !isnan(filteredHum)) {
    display.setCursor(1, 95);  display.print((int)filteredTemp); display.print((char)247); display.print(F("C"));
    display.setCursor(1, 110); display.print(F("H:")); display.print((int)filteredHum); display.print(F("%"));
  } else {
    display.setCursor(1, 95);  display.print(F("--")); display.print((char)247); display.print(F("C"));
    display.setCursor(1, 110); display.print(F("H:--%"));
  }
  display.display(); 
  delay(1000); // Giữ hiển thị đầy đủ thêm 1 giây cho đủ nhịp khởi động rồi qua kết nối mạng

  // 3. TIẾN HÀNH KẾT NỐI WIFI
  wm.setConfigPortalBlocking(true); 
  wm.setConfigPortalTimeout(120);   
  wm.setConnectTimeout(30); 
  WiFi.mode(WIFI_STA);
  
  if (!wm.autoConnect("Smart_Satellite_v15")) { ESP.restart(); }
...
void loop() {
  ...
  if (now - lastScrollTimeMay >= 190) {
    lastScrollTimeMay = now; X_may1++; if (X_may1 > 32) X_may1 = -16; X_may2++; if (X_may2 > 32) X_may2 = -16; 
  }

  if (hasFirstSyncEver && !reconnectingWiFi && (now - lastSyncWiFi > syncInterval)) { 
    reconnectingWiFi = true; reconnectStart = now; wm.autoConnect("Smart_Satellite_v15"); 
  }

  if (reconnectingWiFi) {
    if (now - reconnectStart > 15000) { reconnectingWiFi = false; lastSyncWiFi = now; WiFi.disconnect(true); }
  ...
  if (hasFirstSyncEver && (now - thoiGianCapNhatMangCuoi > timeoutMatMang)) { coDuLieuOnline = false; }
  ...
  if (now - lastDisplayRender >= 33) {
    ...
    if (now - lastShiftTime > 30000) { 
      burnShiftX = random(-1, 2); burnShiftY = random(-1, 2); textShiftX = random(0, 2); iconShiftX = random(-1, 2); iconShiftY = random(-1, 2); lastShiftTime = now;
    }
    ...
    if (!hasFirstSyncEver || isWithinTenSec) {
      if (blinkPhase) { display.setCursor(4 + textShiftX, 5); display.print(F("SYNC")); }
    } else {
      if (hasFirstSyncEver) {
    ...
    // --- TẦNG 2: TRUNG TÂM (CORE CLOCK / CHỮ CHẠY CONNECTING... EARTH) ---
    if (hasFirstSyncEver) {
      ...
      display.setTextSize(1);
      String chuoiConn = F("CONNECTING... EARTH");
      int doRongConn = chuoiConn.length() * 6;
      if (now - lastScrollTimeConn >= 80) { lastScrollTimeConn = now; X_chuConnecting--; if (X_chuConnecting <= -doRongConn) X_chuConnecting = 32; }
      display.setCursor(X_chuConnecting + burnShiftX, 52 + burnShiftY); 
      display.print(chuoiConn);
    }
